...
class Sender:
    ...
    def __init__(self, creds, reader, template, console):
        """
        ...
        try:
            ...
            while mail_list:
                ...
                try:
                    ...
                    self.console.log(f"Mail sent to {receiver[0]}")
                    cursor += 1
                    self.progressbar['value'] = round((cursor/self.mail_size)*100)
                    ...
                    self.console.log("Task Incomplete\n")
                    console.log("Task Incomplete\n")
                    console.close()
                    break
            else:
    ...
    def send(self):
        ...
        try:
            ...
            self.cache_console = Console(os.path.dirname(self.reader_path))
            mail_list = list(self.binder.parse())
            self.mail_size = len(mail_list)

            self._send(self.cache_console, 0, mail_list)
        except Exception as e:
            self.console.log(f"Error: {e}")
    def resume(self):
        """
        ...
        try:
            ...
            self.cache_console = ResumeConsole(
                os.path.basename(self.cachefile.name), os.path.dirname(self.reader_path)
            )
            
            self.cachefile.close()
            self.cache_console.log("Task Resumed")
            self.console.log("Task resumed")
            self._send(self.cache_console, cursor, mail_list)
        except Exception as e:
            self.console.log(f"Error: {e}")
